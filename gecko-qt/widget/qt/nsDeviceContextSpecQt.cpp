                                          ...
                                          bool aIsPrintPreview)
{
    ...
    mToPrinter = aPS->GetOutputDestination() ==
...
RefPtr<PrintEndDocumentPromise> nsDeviceContextSpecQt::EndDocument()
{
    if (mToPrinter) {
        return PrintEndDocumentPromise::CreateAndReject(NS_ERROR_NOT_IMPLEMENTED,
                                                        __func__);
    }
    ...
    return nsIDeviceContextSpec::EndDocumentAsync(
            ...
            [destFile = std::move(destFile),
             spoolFile = std::move(mSpoolFile)]() -> nsresult {
                ...
                nsresult rv = destFile->GetLeafName(destLeafName);
                ...
                // NOT to set in the permissions (thats because files and directories
                // have different numbers of bits for their permissions)
